...
from control.airframe import mass_properties
from control.baseline_v2 import baseline_params, parameter_hash, PARAMETER_SHA256
from control.light_rocket_curve import make_curve_rocket
from control.serialization import jsonable
from control.dynamics import AxialDronePlant, _body_aerodynamics, _rotor_forces_moments, compute_allocation_matrix
from control.geometry import (rotor_thrusts, rotor_wrench, rotor_speeds_for_thrust,
                              control_effectiveness, allocate_wrench, hover_quaternion)
from control.propeller_curve import coefficients, values_and_derivatives, symbolic_force_torque, domain_status
from control.trim import find_trim
from control.curve_authority import nonlinear_trim_authority
...
def test_lqr_reproduces_archived_nominal(p,tmp_path):
    from control.run_baseline_comparison import Factory,run_case
...
def test_imports_are_self_contained():
    ...
    for path in (ROOT/'control').glob('*.py'):
        if path.stem != '__init__':
            importlib.import_module('control.'+path.stem)
